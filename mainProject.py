...
def transaction(identity, amount, choice):  # choice 1 for deposit; choice 2 for withdraw
    ...
    # Append the transaction to the transaction log
    today = date.today()
    transaction_date = today.strftime("%d/%m/%Y")
    with open(f"./database/Customer/{identity}_transactions.txt", "a") as log_file:
        log_file.write(f"{transaction_date},{transaction_type},{amount},{balance}\n")
...
class TransactionLog:
        def __init__(self, window=None, customer_id=None):
            self.master = window
            self.master.title("Transaction Log")
            self.master.geometry("500x400")

            self.label = tk.Label(self.master, text="Transaction History", font=("Segoe UI", 14, "bold"))
            self.label.pack(pady=10)

            # Adding a scrollable Text widget
            self.text = tk.Text(self.master, wrap="word", font=("Segoe UI", 10))
            ...
            self.load_transactions(customer_id)

        def load_transactions(self, customer_id):
            try:
                with open(f"./database/Customer/{customer_id}_transactions.txt", "r") as log_file:
                    transactions = log_file.readlines()
                    self.text.insert(tk.END, "Date       | Type       | Amount | Balance\n")
                    self.text.insert(tk.END, "-" * 40 + "\n")
                    for transaction in transactions:
                        date, type_, amount, balance = transaction.strip().split(',')
                        # Format output with consistent spacing
                        self.text.insert(tk.END, f"{date:<10} | {type_:<10} | {amount:<6} | {balance}\n")
            except FileNotFoundError:
...
class customerMenu:
    def __init__(self, window=None):
        ...
        # self.Button6.place(relx=0.04, rely=0.683, height=34, width=181, bordermode='ignore')
        # self.Button7 = tk.Button(self.Labelframe1, activebackground="#ececec", activeforeground="#000000",
        #                          background="#39a9fc", borderwidth="0", disabledforeground="#a3a3a3",
        #                          font="-family {Segoe UI} -size 11", foreground="#fffffe",
        #                          highlightbackground="#d9d9d9", highlightcolor="black", pady="0",
        #                          text="Transaction Log", command=self.viewTransactionLog)
        # self.Button7.place(relx=0.04, rely=0.8, height=34, width=181, bordermode='ignore')
        ...
        # Transaction Log button with adjusted rely to add spacing
        self.Button7 = tk.Button(self.Labelframe1, activebackground="#ececec", activeforeground="#000000",
                                 background="#39a9fc", borderwidth="0", disabledforeground="#a3a3a3",
                                 font="-family {Segoe UI} -size 11", foreground="#fffffe",
                                 highlightbackground="#d9d9d9", highlightcolor="black", pady="0",
                                 text="Transaction Log", command=self.viewTransactionLog)
        self.Button7.place(relx=0.35, rely=0.683, height=34, width=181, bordermode='ignore')
    ...
    def viewTransactionLog(self):
        TransactionLog(tk.Toplevel(self.master), customer_accNO)
